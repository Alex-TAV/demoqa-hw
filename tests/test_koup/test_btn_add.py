...
def test_koup_add(browser):
    ...
    assert koup_add.btns_delete.check_count_element(4)

    # Проверка названия кнопки для всех элементов (поочерёдно проверяем каждую кнопку)
    for element in koup_add.btns_delete.find_elements():
        assert element.text == 'Delete'

    # Проверка только для первого элемента (Не правильная проверка, проверяет только 1ую кнопку)
    assert koup_add.btns_delete.get_text() == 'Delete'

    # Кликнуть на каждую кнопку "Delete"
    while koup_add.btns_delete.exist(): # Цикл проверяет что существует кнопка. Цикл работает пока условие выполняется (True) т.е. когда закончатся кнопки, цикл закончится
        koup_add.btns_delete.click()

    # Проверка, что кнопок больше нет
